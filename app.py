...
def call_python_script(script: str, timeout_sec: int = 90) -> str:
    """
    Run python code in a fresh python:3.11 container.
    Returns stdout (decoded utf-8). stderr is not returned.
    """
    ...
    try:
        ...
        for chunk in output:
            ...
            loop += 1
        # Note: docker SDK exec has no built-in timeout; outer request enforces total time.
...
async def handle_request(files: List[UploadFile]):
    ...
    with tempfile.TemporaryDirectory() as workdir:
        ...
        # 2) Parse and run python blocks
        try:
            plan = json.loads(planning_json)
        except Exception:
            # Fallback: treat model output as a single Python script
            plan = {"steps": [], "python_blocks": [{"filename": "run.py", "code": planning_json}], "final_format": "array"}

        interim = {}

        # Ensure working dir context for scripts when they read local files:
        # We prepend "import os; os.chdir('<workdir>')" to each block to ensure relative paths work.
        for i, block in enumerate(plan.get("python_blocks", [])):
            ...
            # Expect a single JSON dict on stdout
            try:
        ...
        # 3) Assemble final answer exactly as requested
        assembler_system = (
        ...
        # Try to return JSON if applicable
        try:
            final_obj = json.loads(final_text)
            return JSONResponse(final_obj)
        except Exception:
            # If it isn't JSON, return as text (the grader may expect JSON; this will surface issues)
